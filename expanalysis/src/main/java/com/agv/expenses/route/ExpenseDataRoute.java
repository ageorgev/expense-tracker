...
public class ExpenseDataRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:processSanthomeTransactions")
                .pollEnrich().simple("file:{{data.input.folder}}?fileName={{santhome.excel.file.name}}&noop=true")
        ...
        from("direct:processSBIPDFTransactions")
                ...
                .setHeader("CamelPdfDecryptionMaterial", constant(new StandardDecryptionMaterial("42715220579")))
                .convertBodyTo(byte[].class)
                .to("pdf:extractText")
                // .log("Extracted Body: ${body}")
